...
public class GameController : MonoBehaviour
{
    ...
    public void UpdatePlayersMoney()
    {
        ...
        {
            // Проверьте, существует ли игрок с индексом i
            if (i < allPlayers.Count)
            {
                // Получите игрока с индексом i
                Player player = allPlayers[i];

                // Установите текст в соответствии с количеством денег текущего игрока
                playerTexts[i].text = "Player " + (i + 1) + ": " + player.money.ToString() + "$";
            }
            else
            {
                // Если игрок с данным индексом отсутствует, установите текст как пустую строку
                playerTexts[i].text = "";
            }
        }
    }
